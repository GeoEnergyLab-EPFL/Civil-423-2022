...
l_area = floor(2*Radius);    % length of observed area
d_area = floor(2*Radius);    % Depth of observed area

nrad = 100;      % number of segments in the tip
nfr = 500;

tip_zone = flip([Radius-flip(logspace(-5,-1,nrad));
            -d_area*ones(1,nrad)],2)';
...
figure(fig_ind)
trisurf(meshE.connectivity(:,1:3),meshE.nodes(:,1),meshE.nodes(:,2),S(:,1))
...
inj_point_disp = zeros(n_step,1);
ind_inj = find(meshE.nodes(:,2)==-d_area & meshE.nodes(:,1)==0);
inj_point_disp(1) = U_u(ind_inj * 2);
...
i = 1;
while i<n_step && inj_point_disp(i)>0
    ...
    F_n([eq_free_p+ntot_E;ntot+1])=pn_lhs;
    
    % Solve the system
    %D_x(eq_free)=TotMat(eq_free,eq_free)\F_tot(eq_free)';
    ...
    inj_point_disp(i) = U_n(ind_inj * 2);
...
figure(11)
for i = [1:15:ind]%length(opening_profile(:,1))/2
...
semilogx(t_k(1:ind-1).*c./r_change(1:ind-1).^2,...
    inj_point_disp(1:ind-1)*g./(p_f*r_change(1:ind-1)),'LineWidth',2.5)
