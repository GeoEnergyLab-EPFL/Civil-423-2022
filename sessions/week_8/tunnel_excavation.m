...
bottom = find(meshE.nodes(:,2)==-d_area);
...
tunnel=find(meshP.nodes(:,2)<0 & ...
    round(sqrt((meshP.nodes(:,1)-c_t(1)).^2.+...
    (meshP.nodes(:,2)-c_t(2)).^2),4)==Radius);
...
% Get the solution
AnalyticSolUx= -(Po.*Radius./(2.*g)+(3-4*nu_u).*Radius.*So./(2.*g));
